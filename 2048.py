...
class Cell(Canvas):
	...
	def __init__(self, master, coord):
		...
		self.coord = coord
		self.num = 0
		self.backg = self.round_rectangle(5, 5, self.length-5, self.length-5, r = 25, fill=self.bgs[0])
		self.text = self.create_text(self.length/2,self.length/2,anchor = "center", text="", fill='red', font = ("Arial", 24))
...
class Game(Frame):
	...
	def end_game(self):
		...
		messagebox.showinfo('2048','Game over. You had a final score of: '+str(self.score))
		# self.close()
